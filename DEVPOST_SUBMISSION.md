...
### ⚙️ What It Does
1. **Intelligent Multimodal Ingest (`gemini-3.5-flash`)**: Parses invoice scans, PDFs, and raw OCR text into strict, mathematically verified financial JSON schemas.
...
3. **Autonomous ReAct Audit (`gemini-3.5-pro`)**: Implements a multi-turn Reason + Act loop that queries the chart of accounts, audits tax compliance, maps debits and credits, and detects mismatches.
...
### 🛠️ How We Built It
* **AI Models**: `gemini-3.5-pro` (ReAct reasoning), `gemini-3.5-flash` (multimodal extraction), `gemma-2-2b-it` (PII redaction).
...
* **Multi-Model Google Fleet**: Seamless pipeline orchestrating **Gemma 2 2B IT $\rightarrow$ Gemini 3.5 Flash $\rightarrow$ Gemini 3.5 Pro**.
...
| **0:45 - 1:30** | **Architecture & Gemma Privacy Shield** | Show Mermaid diagram in `README.md`. Demonstrate how `gemma-2-2b-it` strips bank routing/SSN details into tokens before Gemini 3.5 Pro audits the transaction. |
...
>
> An enterprise-grade, zero-trust accounting fleet built with **Gemini 3.5 Pro**, **Gemini 3.5 Flash**, **Gemma 2**, and **Model Context Protocol (MCP)** on **Google Cloud Run & Cloud SQL**.
>
